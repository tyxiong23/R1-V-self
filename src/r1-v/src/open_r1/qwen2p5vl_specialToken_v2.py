...
def get_dist_square_vector(gt_idx, total_grid):
    edge = int(sqrt(total_grid))
    assert edge * edge == total_grid
    gt_x = gt_idx // edge
    gt_y = gt_idx % edge

    dist2_matrix = torch.zeros((edge, edge))
    for i in range(edge):
        for j in range(edge):
            dist2_matrix[i,j] = (i-gt_x)**2 + (j-gt_y)**2
            
    # print(dist2_matrix)
    
    dist2_vector = dist2_matrix.flatten(0)
    assert dist2_vector[gt_idx] == 0
    return dist2_vector


def reward_func_v0_modified(dist2, sigma = 1):
    dist = torch.exp(-dist2 / (2 * sigma ** 2))
    return dist
...
class Qwen2_5_VLForConditionalGenerationSpecialTokenV2(Qwen2_5_VLForConditionalGeneration):
    ...
    def grounding_token_soft_ce_loss(self, logits, labels):
        ...
        num_grounding_tokens = len(grounding_token_ids)
        ...
        for idx, label in enumerate(shift_labels_grounding):
            ...
            # else:
            gt_idx = label - grounding_token_start_id
            
            # dist = torch.exp(-torch.square(dist) / (2 * std_dev ** 2))
            if self.config.reward_func == 'v0':
                
                dist2_vector = get_dist_square_vector(gt_idx, num_grounding_tokens)
                weight = reward_func_v0_modified(dist2_vector, sigma=0.7).to(shift_logits.device)
            else:
                raise NotImplementedError

            weight = weight / weight.sum()
            # print(weight.argmax(), weight.max())
            # print(label - grounding_token_start_id, label, dist)
            gt_labels_groundings[idx][grounding_token_ids] = weight * self.config.grounding_token_ce_lambda
