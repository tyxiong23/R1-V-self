...
DATA_NAME="pixmo-point-counting_filterV1_train-holdout-geminiV0"
...
DATA_PATH="/home/tixiong/storage/xty-workspace/multimodal-reasoning/datasets/pixmo-points/parquet/pixmo-point-counting_filterV1_train_holdout-geminiV0"
...
ITER=3cccccbtjgfcjddbgbtdkgdgjjngfrebgeecngkglrirdc c c c c b t j g f c j e t h b n00
MODEL_PATH="/home/tixiong/storage/xty-workspace/multimodal-reasoning/R1-V/outputs/v3p8_warmup-mix-balanced/warmup+grpo-consistency/pixmo-point-counting_filterV1_train-holdout-geminiV0/grpo-consistency_Qwen2-VL-2B-Instruct_pixmo-point-counting_filterV1_train-holdout-geminiV0_maxlen512_numgen8_epoch1_lr${LR}_formatAlpha1.0_consistency_warmup-gemini-v0-mix-balanced-epoch1/checkpoint-${ITER}"
RUN_NAME="grpo-consistency-resetIter${ITER}-pixmo_${MODEL_SHORT}_${DATA_NAME}_maxlen${MAX_COMPLETION_LEN}_numgen${NUM_GEN}_epoch${EPOCH}_lr${LR}_formatAlpha${FORMAT_REWARD_ALPHA}_consistency_warmup-gemini-v0-mix-balanced-epoch1"
OUTPUT_DIR=${WORKDIR}/outputs/v3p8_warmup-mix-balanced/warmup+grpo-consistency/${DATA_NAME}/${RUN_NAME}
