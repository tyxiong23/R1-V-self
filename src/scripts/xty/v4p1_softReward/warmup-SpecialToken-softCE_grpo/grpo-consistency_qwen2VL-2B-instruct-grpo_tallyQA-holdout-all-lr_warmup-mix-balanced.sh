...
RUN_NAME="grpo-consistency_${MODEL_SHORT}_${DATA_NAME}_maxlen${MAX_COMPLETION_LEN}_numgen${NUM_GEN}_epoch${EPOCH}_lr${LR}_formatAlpha${FORMAT_REWARD_ALPHA}_consistency_warmup-gemini-v0-mix-balanced-epoch1-lr${WARMUP_LR}-SpecialToken30-SoftCE"
OUTPUT_DIR=${WORKDIR}/outputs/v4-grpo-softReward/warmup-specialToken30-SoftCE-mix-balanced-lr${WARMUP_LR}+grpo-consistency/${DATA_NAME}/${RUN_NAME}
...
#      bash /lustre/fsw/portfolios/nvr/users/tixiong/xty-workspace/multimodal-reasoning/R1-V/src/scripts/xty/v4p1_softReward/warmup-SpecialToken-softCE_grpo/grpo-consistency_qwen2VL-2B-instruct-grpo_tallyQA-holdout-all-lr_warmup-mix-balanced.sh 1e-6